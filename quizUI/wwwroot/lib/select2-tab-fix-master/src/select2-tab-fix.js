 ...
 * ////nutchyleo////
 * Modify - 20/3/19
 * -click outside and close a dropdown normaly
 * -tab after focus on select2 (blur event) will close a select2
 * -after reselect (focus - blur - focus agian) will open select2
 * -work well on select2-single mode
 * -work well even on one page have multiple select2-single mode
 */
jQuery(document).ready(function ($) {
    ...
    docBody.on('focus', '.select2', function (e) {
        elthis = e.currentTarget;
        var isfocus = $(elthis).is('.select2-container--focus');
        var elSelect = $(this).siblings('select');
        if (elSelect.is('[disabled]') == false && elSelect.is('[data-s2open]') == false) {
            if (isfocus == false && $(this).has('.select2-selection--single').length > 0) {
    ...
    //    $(elthis).children().children().css('border-bottom','2px solid #FED114 !important;')
    //});

    //docBody.on('select2:closing', function (e) {
    //    $(elthis).children().children().css('border-bottom', '1px solid #aaa')
    //});
